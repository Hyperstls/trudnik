...
| Метод | URL | Auth | Роль | Описание | Вход | Ответ |
|-------|-----|------|------|----------|------|-------|
| GET | `/blacklist` | Да | employer* | Список заблокированных трудников | — | HTML blacklist.html |
| POST | `/blacklist/<user_id>` | Да | employer* | Заблокировать трудника | Path: user_id | JSON / редирект |
| POST | `/unblock/<user_id>` | Да | employer* | Разблокировать трудника | Path: user_id | JSON / редирект |

> \* Роль `worker` получает 403 при доступе к ЧС.
...
- Доступен только employer ([`app/blueprints/blacklist.py`](app/blueprints/blacklist.py:9))
...
| 2 | Детали задания | [`job_detail.html`](templates/job_detail.html:1) | `/jobs/<id>` | Все* | Статус-бейдж, карта (Яндекс), описание, прогресс-бар мест, кнопки действий (зависит от роли и статуса), таймер обратного отсчёта, копирование ID |
...
| 4 | Трудники | [`workers.html`](templates/workers.html:1) | `/workers` | Все | Сетка карточек трудников: аватар, имя, рейтинг (★), навыки, оплата, кнопки «Написать», «Пригласить», «Заблокировать», «В избранное» |
...
| `toggleBlock(btn, workerId)` | Заблокировать/разблокировать | `POST /blacklist/<workerId>` или `POST /unblock/<workerId>` |
...
| **Кнопки действий** | Только иконка (44×44 min touch) | Иконка + текст |
...
- [ ] **Кнопки:** на мобильных — только иконка, на десктопе — иконка + текст
...
| 14 | `/workers` | Найти трудника, добавить в избранное | Кнопка меняется на «Удалить из избранного» |
...
| Шаг | Роль | Действие | Ожидаемый результат |
|-----|------|----------|---------------------|
| 1 | Employer | `/workers` → «Заблокировать» трудника | Кнопка меняется на «Разблокировать» |
| 2 | Employer | `/blacklist` → проверить | Трудник в списке |
| 3 | Worker | `/apply/<job_id>` этого работодателя | Ошибка: «Вы в чёрном списке» |
| 4 | Employer | `/blacklist` → «Разблокировать» | Трудник удалён из ЧС |
...
| [`workers.html`](templates/workers.html:68) | POST | `/blacklist/<user_id>` | Заблокировать |
| [`workers.html`](templates/workers.html:68) | POST | `/unblock/<user_id>` | Разблокировать |

# Трудник – контекст проекта (актуально на 03.06.2026)
...
- **`jobs`** – задания (включая `preferred_religion`, `max_workers` – требуемое количество людей)
...
## К# Рекомендации по улучшению «Трудника»

## Производительность и безопасность
- **Кеширование** – для ускорения можно добавить кеширование ответов Supabase (Flask-Caching).
- **Оптимизация запросов** – избегать множественных запросов к Supabase на одной странице.
- **Mobile-first** – весь дизайн уже адаптирован под мобильные устройства.
- **Безопасность** – все переменные окружения хранятся в `.env` (не коммитить!). RLS-политики настроены в Supabase.

## Удобство использования (UX)
- **Горячие клавиши** – для основных действий (создать задание, принять отклик, написать сообщение).
- **Подсказки и тултипы** – для сложных полей (например, «Желаемая оплата – сумма за всю смену»).
- **Автосохранение форм** – чтобы не терять данные при обрыве связи.
- **Система тегов** – для заданий (срочно, требуется опыт, разовая уборка и т.п.).
- **Тёмная тема** – для комфортной работы вечером (переключение в профиле).
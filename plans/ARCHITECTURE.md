...
> ⚠️ **УСТАРЕЛО.** Этот документ больше не поддерживается. Актуальная версия архитектурной документации находится в [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Все новые изменения вносятся только в файлы директории [`docs/`](docs/).

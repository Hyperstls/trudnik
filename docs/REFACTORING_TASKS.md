...
> Дата: 2026-06-22 | **Актуализировано:** 2026-06-27

---

## ✅ Выполненные задачи (июнь 2026)

| # | Задача | Статус |
|---|--------|--------|
| 1 | **Миграция с Supabase на Amvera/PostgREST** | ✅ Выполнено |
| 2 | **Очистка корневых тестовых скриптов** (`test_*.py`) — перемещены в `archive/` | ✅ Выполнено |
| 3 | **Исправление чтения версии в admin.py** — `VERSION` файл вместо `git` | ✅ Выполнено |
| 4 | **Архивирование browser_agent.py, cleanup_extra_users.py, manage_users.py** — перемещены в `archive/` | ✅ Выполнено |
| 5 | **Удаление временных логов Amvera из uploads/** | ✅ Выполнено |
...
| # | Задача | Файл | Действие |
|---|--------|------|----------|
| 55 | Проверить и обновить CI/CD (GitHub Actions) с учётом новой структуры | [`.github/workflows/`](.github/workflows/) | Актуализировать пайплайны: заменить `supabase` на `postgrest`/`amvera` в переменных окружения, обновить пути к скриптам, учесть `archive/` и новую структуру `tests/` |
...
| Фаза 4 | LOW | 17 | Когда будет время |
| **Итого** | | **55** | |

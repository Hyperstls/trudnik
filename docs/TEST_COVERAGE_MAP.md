...
Дата: 2026-08-15 (обновления: 2026-08-16, 2026-08-21). Метод: статический анализ (`@*_bp.route` в `app/blueprints/*.py` → поиск URL-литералов в `tests/test_*.py`; locustfile.py и conftest* НЕ учитываются как тесты) + ручная верификация ложных срабатываний/пропусков чтением кода тестов.

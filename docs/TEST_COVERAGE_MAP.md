...
Дата: 2026-08-15 (обновления: 2026-08-16, 2026-08-21, 2026-09-04, **2026-09-12**). Метод: статический анализ (`@*_bp.route` в `app/blueprints/*.py` → поиск URL-литералов в `tests/test_*.py`; locustfile.py и conftest* НЕ учитываются как тесты) + ручная верификация ложных срабатываний/пропусков чтением кода тестов.

## 📌 Обновление 2026-09-12: integration 100%

**Integration: 171 passed / 0 failed / 0 skipped** (живой docker-стек). Закрыты все data-скипы и фантом-тесты:
- conftest created_job_id: POST /job/new (был /jobs/new → 405) + поля актуальной формы; my-applications?job_id= фильтр (обе вкладки) — тесты берут отклики своего задания, а не «первый попавшийся» (устраняло Accept-400-каскад из 41 скипа).
- INV-009: temp-юзер через /admin/test-user (verified), ID приглашения по job_id.
- Реальные баги, найденные доведением до 100%: **PGRST203-перегрузка register_user** (мигр. 141 создала дубль сигнатуры → /admin/test-user молча падал; фикс — мигр. 143 DROP старой), **invite на закрытое задание проходил** (добавлена проверка status='open', INV-004), my_jobs без сортировки (→ created_at.desc), POST /admin/skills теперь возвращает объект (Prefer: return=representation).
- Обоснование «563 skipped» в mock-режиме: by-design авто-скип integration-модулей при недостижимом стеке (TEST_BASE_URL закрыт); при живом стеке все 171 выполняются и проходят.

Прогоны 2026-09-12: mock 553 ✓ / integration 171 ✓ (0 skip) / e2e 113 ✓ / pre_deploy 0.
